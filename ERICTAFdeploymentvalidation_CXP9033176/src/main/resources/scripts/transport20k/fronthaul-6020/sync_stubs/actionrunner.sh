...
expect "]]>]]>"
send "<rpc message-id=\"${MSG_ID}\" xmlns=\"urn:ietf:params:xml:ns:netconf:base:1.0\"><get><filter><ManagedElement xmlns=\"urn:com:ericsson:ecim:ComTop\"><managedElementId>1</managedElementId>                    </ManagedElement></filter></get></rpc>]]>]]>\r"
...
# Non TLS node
echo "TLS 1 node, tls disabled, ssh"
expect <<EOF
set timeout 60
if { $LOG_ENABLED == true } { log_file -noappend ${LOGS_DIR}/${JOB_ID}_${NE_NAME}.log }
spawn ssh -o StrictHostKeyChecking=no admin@$NE_IP -p $SSH_PORT -s netconf
expect "password:"
send "admin\r"
expect "]]>]]>"
send "<rpc message-id=\"${MSG_ID}\" xmlns=\"urn:ietf:params:xml:ns:netconf:base:1.0\"><get><filter><ManagedElement xmlns=\"urn:com:ericsson:ecim:ComTop\"><managedElementId>1</managedElementId>                    </ManagedElement></filter></get></rpc>]]>]]>\r"
