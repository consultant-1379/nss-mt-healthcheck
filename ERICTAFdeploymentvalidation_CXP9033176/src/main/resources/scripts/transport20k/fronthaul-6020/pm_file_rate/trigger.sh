...
echo sarstop.sh $NETSIM_SERVER $SARPID $Job_id $NUM_NODES $runtime
sh sarstop.sh $NETSIM_SERVER $SARPID $Job_id $NF $runtime
...
echo "---------------------------------------------" >> results.txt
echo "--" >> finalFile.txt
echo "$(cat finalFile.txt) $NF $Diff $count" > finalFile.txt
