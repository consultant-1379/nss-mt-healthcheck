...
{
    `cat nodeips.txt | head -n $NUM_NODES > selectednodes.txt`
    echo "Siu Upgrade triggered for $NUM_NODES" >> siuupgradestats.log
    while read -r NODE_NAME NODE_IP;
    do
        software_upgrade_siu $NODE_NAME $NODE_IP $JOB_ID $NETSIM_SERVER 
    done < selectednodes.txt
    parallel_wait
    parallel_status
}

function software_upgrade_siu ()
{
    LOG_FILE=software_upgrade_siu_${NODE_NAME}.log
    STATUS_HEADER=software_upgrade_siu_${NODE_NAME}
    (
    (
        echo "Running the function software_upgrade_siu_${NODE_NAME}"
        ./siutcuswupgrade.sh ${NODE_NAME} ${NODE_IP} ${JOB_ID} ${NETSIM_SERVER}
    ) > $LOG_FILE 2>&1; parallel_finish
    ) & set_parallel_variables
}
...
function get_pm_files ()
{
    ...
    while read -r SIM_NAME NODE_NAME NODE_IP AGENT_PORT COMMUNITY;
    do
	echo get_pm_file $NODE_NAME $NODE_IP $AGENT_PORT $COMMUNITY
        get_pm_file $NODE_NAME $NODE_IP $AGENT_PORT $COMMUNITY
	#get_pm_file2 $NODE_NAME $NODE_IP $AGENT_PORT $COMMUNITY
    done < selectednodes.txt
    parallel_wait
    parallel_status
}
...
function get_pm_file ()
{
    ...
    (
        ...
        ./ml6352_pm.sh ${NODE_NAME} ${NODE_IP} ${AGENT_PORT} ${COMMUNITY}
    ) > $LOG_FILE 2>&1; parallel_finish
    ) & set_parallel_variables
}
function get_pm_file2 ()
{
    LOG_FILE=get_pm_file2_${NODE_NAME}.log
    STATUS_HEADER=get_pm_file2_${NODE_NAME}
    (
    (
        echo "Running the function get_pm_file2_${NODE_NAME}"
        ./ml6352_pm_24.sh ${NODE_NAME} ${NODE_IP} ${AGENT_PORT} ${COMMUNITY}
    ) > $LOG_FILE 2>&1; parallel_finish
    ) & set_parallel_variables
}

function get_outdoor_pm ()
{
    `cat nodeips.txt | head -n $NUM_NODES > selectednodes.txt`
    while read -r NODE_NAME NODE_IP;
    do
        get_outdoor_pm_single $NODE_NAME $NODE_IP
    done < selectednodes.txt
    parallel_wait
    parallel_status
}

function get_outdoor_pm_single ()
{
    ...
    (
        ...
        ./outdoorpm.sh ${NODE_IP} ${NODE_NAME}
    ) > $LOG_FILE 2>&1; parallel_finish
    ) & set_parallel_variables
}


function genstats_outdoor_pm ()
{
    `cat nodeips.txt | head -n $NUM_NODES > selectednodes.txt`
    while read -r NODE_NAME NODE_IP;
    do
        genstats_outdoor_pm_single $NODE_NAME $NODE_IP
    done < selectednodes.txt
    parallel_wait
    parallel_status
}

function genstats_outdoor_pm_single ()
{
    LOG_FILE=get_pm_counter_${NODE_NAME}.log
    STATUS_HEADER=get_pm_counter_${NODE_NAME}
    (
    (
        echo "Running the function get_pm_counter_${NODE_NAME}"
        ./testgenstat.sh ${NODE_NAME} ${NODE_IP}
        parallel_finish
    )
    ) & set_parallel_variables
}
