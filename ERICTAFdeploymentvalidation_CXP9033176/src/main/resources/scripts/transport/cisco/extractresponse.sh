...
#	fullStr=$fullStr$line
		...
		if [[ "$fullStr" == *get-response* ]];then
			#echo $fullStr
			Date=$(echo $fullStr | awk -F\{ '{print $4}')
			Date=${Date%??}
			Time=$(echo $fullStr | awk -F\{ '{print $5}')
			Time=${Time%??????}
                        ResponseId=$(echo $fullStr | awk -F"," '{print $13}'| awk -F"}" '{print $1}')
	...
	#		echo $nodeName
	        	echo $CurrDate$space$Date$space$Time$space$nodeName$space$Msec1$space$ResponseId >> $result 
...
if [[ "$fullStr" == *prmnresponse* ]];then
		...
		#	ss1=$(echo $Time | cut -d',' -f3).$(echo $Time | cut -d',' -f4)
			CurrDate=$(date -d"$yyy1-$mmm1-$ddd1 $hh1:$mm1:$ss1" +%s%3N)
		        nodeName=$(echo $fullStr | awk -F\" '{print $2}')
	        	echo $CurrDate$space$Date$space$Time$space$nodeName$space$ss1$space$ResponseId >> $result 
