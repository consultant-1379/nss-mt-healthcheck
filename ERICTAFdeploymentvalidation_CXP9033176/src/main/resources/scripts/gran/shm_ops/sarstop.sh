...
cat $FILENAME | sed 1,3d |awk -v var1=$NUM_NODES -v var2=$NUM_TIMES -v var3=$RUN_TIME 'var=100-$8 {sum += var; if(var>max){max=var}; if(var==100){fullcount +=1}} END {printf "NumNodes: %d ::Num of times: %d  ::Runtime: %d ::Average: %f ::Max: %f ::100%CPU: %d\n", var1, var2,var3,sum/NR, max, fullcount}' >> totalstat.log
