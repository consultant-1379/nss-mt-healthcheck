...
#Usage : sh.trigger.sh <NETSim Server>  <Num of Nodes> <Num of sessions>
# sh .trigger.sh ieatnetsimv5109-93 1 10

################################################################################

function usage()
{
	echo Please Enter the valid inputs
        exit
}
...
function gsmupgrade ()
{
    ...
    do
	echo gsm $NUM_TIMES
        gsm_upgrade $NODE_NAME $NODE_IP $NUM_TIMES
...
function gsm_upgrade ()
{
   ...
   do
	gsm_upgrade_single ${NODE_NAME} ${NODE_IP} ${i}
	let i=$i+1
        echo "Triggered $i time"
   done
}

function gsm_upgrade_single ()
{
    LOG_FILE=gsm_upgrade_single_${NODE_NAME}_${i}.log
    STATUS_HEADER=gsm_upgrade_single_${NODE_NAME}_${i}
    (
    (
        #echo "Running the function gsm_upgrade_${NODE_NAME} ${NUM_TIMES}"
      	echo gsm $NUM_TIMES $NODE_NAME $NODE_IP
	sh opscommandexecutor.sh ${NODE_NAME} ${NODE_IP}
...
sh sarstop.sh $NETSIM_SERVER $SARPID $JOB_ID $NUM_NODES $runtime $NUM_TIMES
...
echo "Script execution took $runtime milli seconds for ${NUM_NODES} nodes and  $NUM_TIMES sessions"
echo "Return code $EXIT_CODE, Script execution took $runtime milli seconds for ${NUM_NODES} and ran for $NUM_TIMES sessions" >> total_ops.log
