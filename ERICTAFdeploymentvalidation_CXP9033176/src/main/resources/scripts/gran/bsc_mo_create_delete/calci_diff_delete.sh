...
sh extract_delete_prmn.sh $file $result $NE_NAME
...
echo $NE_NAME The number of MO Created:- $n_MO Usecase1:- $difference MO/per sec is : $MO_per_sec and NOTIF/per sec is : $NOTIF_per_sec >> observations_delete.txt
done < $nodefile
