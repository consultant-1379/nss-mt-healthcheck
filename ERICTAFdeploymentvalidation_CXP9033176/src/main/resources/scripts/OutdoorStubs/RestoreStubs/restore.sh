...
set x [clock milliseconds]
send "config common cdb restore filename $path/Original_backup.cdb mode sftp ip $hostIP port $port user $user password $password\r"
expect -re "#"
set y [clock milliseconds]
set diff [expr $y-$x]
...
for {set nodeStartTimeout 1} {$nodeStartTimeout < $nodeTimeout} {incr nodeStartTimeout 1} {
	...
	expect {
	-re "RESTORING" { send_user "\r Restoring in process \r" }
	-re "COMPLETE" {send_user "\r######### File Download Complete ##########"; set download 1; set flag 1; break }
        }
}
...
if { $download == 0 } {
send_user "Download Timed out"
puts $rf "Restore failed for $NODE_NAME"
exit 1
}
