...
function brmbackup ()
{
    ...
    do
        backup $SIM_NAME $NODE_NAME $NODE_IP $PROTOCOL $JOB_ID $SERVER
...
function backup ()
{
    ...
    (
        ...
        ./brmbackup.sh -s ${SIM_NAME} -n ${NODE_NAME} -i ${NODE_IP}
...
function swupgrade ()
{
    ...
    while read -r SIM_NAME NODE_NAME NODE_IP;
    do
        software_upgrade $SIM_NAME $NODE_NAME $NODE_IP $JOB_ID $SERVER
    done < selectednodes.txt
    parallel_wait
    parallel_status
    if [[ $? -ne 0 ]]
...
function software_upgrade ()
{
    ...
    (
        ...
        ./swupgrade.sh -s ${SIM_NAME} -n ${NODE_NAME} -i ${NODE_IP}
    ) > $LOG_FILE 2>&1; parallel_finish
    ) & set_parallel_variables
}
...
if [[ $FUNCTION == "amos" || $FUNCTION == "amos_ssh" || $FUNCTION == "amos_combined" ]]
then
	mintime=`cat ${FUNCTION}_stats.log | awk -F":::" '{print $2}' | awk '{if(var<min){min=var}} END {print $min}'`
	reqtime=`expr $mintime + 60`
	noofcommands=`cat ${FUNCTION}_stats.log | awk -F":::" '{print $4}' |  awk -v var1=$reqtime '{var=$1;if(var<var1){fullcount += 1}} END {printf "%d\n",fullcount}'`
	echo "num_nodes - $NUM_NODES - no_commands - $noofcommands" >> amoscommandstat.log
fi
