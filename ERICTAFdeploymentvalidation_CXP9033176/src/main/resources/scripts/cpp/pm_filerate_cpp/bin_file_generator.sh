...
## NOTE - The above mkdir should commented when re triggering the file or the directory is already created..
cd LTE03/
q=1
k=1
	while [[ $q -le 7 ]]
		...
		name2=$name$i$q
		ln -sf netsim_users/pms/rec_templates/celltrace_2304K.bin.gz $name2
		q=`expr $q + 1`
	done
        while [[ $q -le 7 ]]
        do
            ...
            name2=$name$i$k
            ln -sf /netsim_users/pms/rec_templates/celltrace_768K.bin.gz $name2
            k=`expr $k + 1`
        done
done
