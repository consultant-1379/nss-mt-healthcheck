...
# Step -2 : Extract insights
	...
	startNE=`grep $NE ${REQUEST_FILE}_sort | cut -d" " -f1`
	endNE=`grep $NE ${RESPONSE_FILE}_sort | cut -d" " -f1` 
	timetakenNE=`expr $endNE - $startNE`
	total=`awk "BEGIN {print (${timetakenNE}/1000000)}"`
	echo "$NE $startNE $endNE $total" >> "${JobId}.csv"
    done<$jobfile
	wait
	...
	timePerNodeSec=`awk "BEGIN {print (${timePerNode}/1000000)}"`
        echo "Total Time : ${totalTime}" >> "${JobId}.csv"
	echo "Node Count : ${totalNodes}" >> "${JobId}.csv"
	echo "Time taken per node : ${timePerNodeSec}"  >> "${JobId}.csv"
