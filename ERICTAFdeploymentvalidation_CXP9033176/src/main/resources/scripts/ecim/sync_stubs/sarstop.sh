...
cat $FILENAME | sed 1,3d |awk -v var1=$NUM_NODES  -v var3=$RUN_TIME 'var=100-$8 {sum += var; if(var>max){max=var}; if(var==100){fullcount +=1}} END {printf "NumNodes: %d  ::Runtime: %d ::Average: %f ::Max: %f ::100%CPU: %d\n", var1,var3,sum/NR, max, fullcount}' >> totalstat.log
totalStat="totalstat.log"
/usr/bin/expect - << "EOF" $SERVER $totalStat

set SERVER [lindex $argv 0]
set totalStat [lindex $argv 1]
spawn scp -o StrictHostKeyChecking=no $totalStat netsim@${SERVER}:/netsim/CM_SYNC_DG2
expect {
        -re "assword:" { send "netsim\r"; exp_continue }
